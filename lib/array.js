...
internals.checkItems = function (items, wasArray, state, options) {
    ...
    for (var v = 0, vl = items.length; v < vl; ++v) {
        ...
        for (var i = 0, il = this._inner.contains.length; i < il; ++i) {
            var res = this._inner.contains[i]._validate(item, localState, options);
            if (!res.errors) {
                items[v] = res.value;

                if (contains.indexOf(i) === -1) {
                    contains.push(i);
                    break;
                }
            }
        ...
        for (i = 0, il = this._inner.exclusions.length; i < il; ++i) {
    ...
    if (contains.length !== this._inner.contains.length) {
        errors.push(Errors.create('array.contains', null, { key: state.key, path: state.path }, options));
    }
...
internals.Array.prototype.contains = function () {

    var contains = Hoek.flatten(Array.prototype.slice.call(arguments)).map(function (type) {

        return Cast.schema(type);
    });

    var obj = this.clone();
    obj._inner.contains = obj._inner.contains.concat(contains);
    return obj;
};
